<?php
    ...
    // We can use php command line instead of browser
    ...
    foreach($xmlcont as $frame) 
    {
        if((intval($frame->time) >= 44) && (intval($frame->time) <= 79)) { // part1
            ...
            $imgFace = transform(-2, 2.42*$face_w * $ratio_800_1280, 2.42*$face_h * $ratio_800_1280, $imgFace);

            // $face = new Imagick("RESULT/frame_0".$frame->time. ".png");
            $frm = new Imagick("frames_800/frame_0".$frame->time. ".png");
            $frm->compositeImage($imgFace, imagick::COMPOSITE_DSTOVER, intval($frame->x +92.4 -241)*$ratio_800_1280, intval($frame->y -20.8 -277)*$ratio_800_1280); 
            ...
            $frm = new Imagick("frames_800/frame_0".$frame->time. ".png");
            // $face = new Imagick("RESULT/frame_0".$frame->time. ".png");
            $frm->compositeImage($imgFace, imagick::COMPOSITE_DSTOVER, intval(($frame->x +62 -241)*$ratio_800_1280), intval(($frame->y -32.3 -277)*$ratio_800_1280)) ; 
            $img_outp = $face_dir."/frame_0". $frame->time. ".png";
            $frm->writeImage(''.$img_outp); 
        } else if((intval($frame->time) >= 132) && (intval($frame->time) <= 196)) { // part3  132 (196)
            // Caused by Adobe After Effect wrong manual proccess generate shifted position.
            $anchor_x = 43.7;
            $anchor_y = 56.6;

            $x_zoom = $frame->scl_x/100;
            $y_zoom = $frame->scl_y/100;
            $scl_x = floatval($x_zoom)*36.2/100 * $face_w* $ratio_800_1280;
            $scl_y = floatval($y_zoom)*39.5/100 * $face_h* $ratio_800_1280;
            ...
            if($frame->rt > 0) {
                $imgBG->compositeImage($imgFace, imagick::COMPOSITE_DSTOVER, intval($new_anchor_x-$shift_x)*$ratio_800_1280, intval($new_anchor_y)*$ratio_800_1280); 
            } else {
                $imgBG->compositeImage($imgFace, imagick::COMPOSITE_DSTOVER, intval($new_anchor_x)*$ratio_800_1280, intval($new_anchor_y +$shift_y)*$ratio_800_1280); 
            }
            $img_outp = $face_dir."/frame_0". $frame->time. ".png";
            ...
            if($frame->rt > 0) {
                $imgBG->compositeImage($imgFace, imagick::COMPOSITE_DSTOVER, intval($new_anchor_x-$shift_x+5)*$ratio_800_1280, intval($new_anchor_y-5)*$ratio_800_1280); 
            } else {
                $imgBG->compositeImage($imgFace, imagick::COMPOSITE_DSTOVER, intval($new_anchor_x+5)*$ratio_800_1280, intval($new_anchor_y +$shift_y-5)*$ratio_800_1280); 
            }
            $img_outp = $face_dir."/frame_0". $frame->time. ".png";
            $imgBG->writeImage(''.$img_outp); 
        }
    ...
    function compose($face_dir) {
        ...
        // /opt/local/bin/ffmpeg in some server
        exec("/usr/bin/ffmpeg -framerate 30 -i $face_dir/frame_0%03d.png -s:v 800x450 -c:v libx264 -profile:v high -crf 23 -pix_fmt yuv420p -r 30 -vb 12M  $face_dir/movies_$name.mp4 1>&2", $o, $v);
        exec("/usr/bin/ffmpeg -i $face_dir/movies_$name.mp4 -c copy -bsf:v h264_mp4toannexb -f mpegts $face_dir.ts", $o2, $v2);
        print_r($v2);

        // Merge face-replaced video with other parts to form lastest video has same length.
        // Here we merge 3 part
        exec("/usr/bin/ffmpeg -i \"concat:intermediate1.ts|$face_dir.ts|intermediate3.ts\" -c copy -bsf:a aac_adtstoasc output_$name.mp4", $o3, $v3);
        exec("rm -rf $face_dir/");
        exec("/usr/bin/ffmpeg -i output_$name.mp4 -i audio.mp3  -strict -2 Final_$name.mp4", $out_aud, $v_aud);
        print_r($out_aud);
        print_r($o3);
    ...
    function add_buffer_frame($face_dir) {
        exec("cp -r buffer_800/* $face_dir 2>&1", $o, $v);
    }
