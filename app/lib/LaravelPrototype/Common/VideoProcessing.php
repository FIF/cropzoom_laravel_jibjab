...
class VideoProcessing {
    ...
    private function _compose($face_dir, $material_path) {
        // Careful set your ffmpeg bin path
        // $ffmpeg = "/usr/bin/ffmpeg";
        $ffmpeg = "/usr/local/bin/ffmpeg";
        $videoPath = $material_path . "videos/";
        $name = rand(1, 99999);
        exec("cp -r {$material_path}buffer_800/frame_0*.png $face_dir/");
        // -preset ultrafast not work on ubuntu
        // -c:v libx264
        exec("$ffmpeg -framerate 30 -i $face_dir/frame_0%03d.png  -s:v 800x450 -c:v libx264 -profile:v high -crf 23 -pix_fmt yuv420p -r 30 -vb 12M  $face_dir.ts");
        // exec("$ffmpeg -i $face_dir/movies_$name.mp4 -c copy -bsf:v h264_mp4toannexb -f mpegts $face_dir.ts");
        exec("$ffmpeg -i \"concat:{$material_path}intermediate1.ts|$face_dir.ts|{$material_path}intermediate3.ts\" -c copy -bsf:a aac_adtstoasc  {$videoPath}output_$name.mp4");
        exec("cp  $face_dir/frame_0392.png {$videoPath}thumbnail_$name.png");
        // exec("rm -rf $face_dir/");
        exec("$ffmpeg -i {$videoPath}output_$name.mp4 -i {$material_path}audio.wav -c:v libx264 -strict experimental  {$videoPath}final_$name.mp4");
